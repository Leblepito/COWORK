...
export async function getUsageSummary(period = "day"): Promise<UsageSummary> {
  return coworkFetch<UsageSummary>(`/usage/summary?period=${period}`)
}

export async function getUsageByAgent(agentId: string) {
  return coworkFetch<Record<string, unknown>[]>(`/usage/by-agent/${agentId}`)
}

export async function getTaskHistory(taskId: string): Promise<TaskHistoryEntry[]> {
  return coworkFetch<TaskHistoryEntry[]>(`/tasks/${taskId}/history`)
}

// ── Budget ──
export interface BudgetStatus {
  total: number; limit: number; percent: number; warning: boolean; exceeded: boolean
}
export async function getUsageBudget(): Promise<BudgetStatus> {
  return coworkFetch<BudgetStatus>("/usage/budget")
}

// ── Filtered Tasks ──
export async function getFilteredTasks(params: {agent?: string, status?: string, from?: string, to?: string} = {}) {
  const qs = new URLSearchParams()
  if (params.agent) qs.set("agent", params.agent)
  if (params.status) qs.set("status", params.status)
  if (params.from) qs.set("date_from", params.from)
  if (params.to) qs.set("date_to", params.to)
  return coworkFetch<CoworkTask[]>(`/tasks?${qs.toString()}`)
}
